...
export const loader = async ({
	...
	const item = await db
		.select()
		.from(expensesTable)
		.where(eq(expensesTable.id, params.item))
		.get();
...
export const action = async ({
	...
	await db
		...
		.set({
			...
			tag_id: String(submission.value.tags),
			note: submission.value.note,
			payer: submission.value.payer,
			paid_at: submission.value.paidAt,
		})
		.where(eq(expensesTable.id, params.item));

	const ym = submission.value.paidAt.split("-").slice(0, 2).join("-");
			...
			>
				<Container size="3" pt="4" style={{ maxWidth: "100%" }}>
					...
					<Button
						...
						onClick={() => {
							navigate(
								`/${household.id}/summary?ym=${item.paid_at.split("-").slice(0, 2).join("-")}`,
							);
						}}
					>
						サマリーに戻る
